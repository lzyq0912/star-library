...
const { assertSupportedNodeVersion } = require('./runtime-version');
const { decryptSecret, encryptSecret, secretHint } = require('./secrets');
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS custom_sources (
    ...
    description TEXT,
    refresh_interval_ms INTEGER NOT NULL DEFAULT 3600000,
    enabled INTEGER NOT NULL DEFAULT 1,
    created_at INTEGER NOT NULL,
    updated_at INTEGER NOT NULL,
    CHECK(category IN ('article', 'news', 'podcast')),
    CHECK(refresh_interval_ms >= 300000)
  );

  CREATE INDEX IF NOT EXISTS idx_custom_sources_enabled
    ON custom_sources(enabled, updated_at DESC);
  ...
  CREATE TABLE IF NOT EXISTS ai_profiles (
    ...
    created_at INTEGER NOT NULL,
    updated_at INTEGER NOT NULL,
    FOREIGN KEY(credential_id) REFERENCES secret_credentials(id) ON DELETE SET NULL
  );
  ...
  CREATE TABLE IF NOT EXISTS ai_settings (
    ...
    rewrite_profile_id TEXT,
    agent_profile_id TEXT,
    updated_at INTEGER NOT NULL,
    FOREIGN KEY(active_profile_id) REFERENCES ai_profiles(id) ON DELETE SET NULL,
    FOREIGN KEY(translation_profile_id) REFERENCES ai_profiles(id) ON DELETE SET NULL,
    FOREIGN KEY(rewrite_profile_id) REFERENCES ai_profiles(id) ON DELETE SET NULL,
    FOREIGN KEY(agent_profile_id) REFERENCES ai_profiles(id) ON DELETE SET NULL
  );
`);
...
const AI_PURPOSE_COLUMNS = {
  active: 'active_profile_id',
  translation: 'translation_profile_id',
  rewrite: 'rewrite_profile_id',
...
function normalizeCustomSourceRow(row) {
  ...
  return {
    ...
    refreshIntervalMs: Number(row.refresh_interval_ms) || 60 * 60 * 1000,
...
function listCustomSources() {
  return db.prepare(`
    SELECT * FROM custom_sources
    ORDER BY created_at ASC, id ASC
  `).all().map(normalizeCustomSourceRow);
}

function getCustomSource(id) {
  return normalizeCustomSourceRow(db.prepare('SELECT * FROM custom_sources WHERE id = ?').get(String(id || '').trim()));
}

function saveCustomSource(source = {}) {
  const id = String(source.id || `custom-${crypto.randomUUID()}`).trim();
  const name = String(source.name || '').replace(/\s+/g, ' ').trim().slice(0, 80);
  const feedUrl = String(source.feedUrl || '').trim().slice(0, 2048);
  const siteUrl = String(source.siteUrl || '').trim().slice(0, 2048);
  const category = ['article', 'news', 'podcast'].includes(source.category) ? source.category : 'article';
  const description = String(source.description || '').replace(/\s+/g, ' ').trim().slice(0, 240);
  const refreshIntervalMs = Math.max(5 * 60 * 1000, Math.min(
    7 * 24 * 60 * 60 * 1000,
    Number(source.refreshIntervalMs) || 60 * 60 * 1000,
  ));
  if (!id || !name || !feedUrl || !siteUrl) {
    const error = new Error('订阅源名称、Feed URL 和站点地址不能为空');
  ...
  try {
    db.prepare(`
      INSERT INTO custom_sources (
        id, name, feed_url, site_url, category, description,
        refresh_interval_ms, enabled, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(id) DO UPDATE SET
        name = excluded.name,
    ...
    if (/UNIQUE constraint failed: custom_sources\.feed_url/i.test(String(error && error.message))) {
      const conflict = new Error('这个 Feed 已经订阅');
      conflict.statusCode = 409;
      throw conflict;
    }
    throw error;
  }
  return getCustomSource(id);
}

function setCustomSourceEnabled(id, enabled) {
  db.prepare('UPDATE custom_sources SET enabled = ?, updated_at = ? WHERE id = ?')
    .run(enabled ? 1 : 0, now(), String(id || '').trim());
  return getCustomSource(id);
}

function deleteCustomSource(id) {
  return db.prepare('DELETE FROM custom_sources WHERE id = ?').run(String(id || '').trim()).changes > 0;
}

function normalizeAiProfileRow(row) {
  if (!row) return null;
  return {
    id: row.id,
    name: row.name,
    ...
    apiKeyUrl: row.api_key_url || '',
    baseUrl: row.base_url,
    model: row.model,
    temperature: Number(row.temperature),
    maxTokens: Number(row.max_tokens),
    credentialId: row.credential_id || '',
    hasApiKey: Boolean(row.credential_id),
    apiKeyMasked: row.secret_hint || '',
...
function aiProfileSelectSql(where = '') {
  return `
    SELECT p.*, c.secret_hint
    FROM ai_profiles p
    LEFT JOIN secret_credentials c ON c.id = p.credential_id
    ${where}
  `;
}

function listAiProfiles() {
  return db.prepare(`${aiProfileSelectSql()} ORDER BY p.is_default DESC, p.created_at ASC`).all().map(normalizeAiProfileRow);
}

function getAiProfile(id) {
  return normalizeAiProfileRow(db.prepare(aiProfileSelectSql('WHERE p.id = ?')).get(String(id || '').trim()));
}

function normalizeAiProfileId(value) {
  const id = String(value || '').trim();
  return /^[A-Za-z0-9_-]{8,96}$/.test(id) ? id : `ai-${crypto.randomUUID()}`;
}

function saveAiProfile(profile = {}) {
  const existing = profile.id ? getAiProfile(profile.id) : null;
  const id = existing ? existing.id : normalizeAiProfileId(profile.id);
  const name = String(profile.name || '').replace(/\s+/g, ' ').trim().slice(0, 80);
  const provider = String(profile.provider || 'custom').replace(/[^A-Za-z0-9_.-]/g, '').trim().slice(0, 48) || 'custom';
  const providerName = String(profile.providerName || name || provider).replace(/\s+/g, ' ').trim().slice(0, 80);
  const providerType = ['openai_compatible', 'anthropic_compatible'].includes(profile.providerType)
    ? profile.providerType
    : 'openai_compatible';
  const providerCategory = String(profile.providerCategory || '').replace(/\s+/g, ' ').trim().slice(0, 80);
  const apiKeyUrl = String(profile.apiKeyUrl || '').trim().slice(0, 2048);
  const baseUrl = String(profile.baseUrl || '').trim().slice(0, 2048);
  const model = String(profile.model || '').replace(/[\r\n]/g, '').trim().slice(0, 160);
  const temperature = Math.max(0, Math.min(2, Number(profile.temperature) || 0));
  const maxTokens = Math.max(1, Math.min(32768, Math.floor(Number(profile.maxTokens) || 2000)));
  const apiKey = String(profile.apiKey || '').trim();
  if (!name || !baseUrl || !model) {
    const error = new Error('配置名称、Base URL 和模型不能为空');
    error.statusCode = 400;
    throw error;
  }
  if (apiKey.length > 4096) {
    const error = new Error('API Key 长度超出限制');
    error.statusCode = 400;
    throw error;
  }
  const t = now();
  let credentialId = existing && existing.credentialId || '';
  db.exec('BEGIN');
  try {
    if (apiKey) {
      credentialId = credentialId || `secret-${crypto.randomUUID()}`;
      const encrypted = encryptSecret(apiKey, { id: credentialId, kind: 'ai-api-key' });
      db.prepare(`
        INSERT INTO secret_credentials (
          id, kind, ciphertext, iv, auth_tag, secret_hint, key_version, created_at, updated_at
        ) VALUES (?, 'ai-api-key', ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(id) DO UPDATE SET
    ...
    const makeDefault = Boolean(profile.isDefault) || (!existing && listAiProfiles().length === 0);
    if (makeDefault) db.prepare('UPDATE ai_profiles SET is_default = 0 WHERE is_default = 1').run();
    db.prepare(`
      INSERT INTO ai_profiles (
        id, name, provider, provider_name, provider_type, provider_category,
        api_key_url, base_url, model, temperature, max_tokens, credential_id,
    ...
    `).run(
      ...
      maxTokens,
      credentialId || null,
      makeDefault ? 1 : (existing && existing.isDefault ? 1 : 0),
      existing && existing.createdAt || t,
      t,
    );
    db.exec('COMMIT');
  } catch (error) {
...
function getAiProfileConfig(id) {
  const profile = getAiProfile(id);
  if (!profile) return null;
  let apiKey = '';
  if (profile.credentialId) {
    const row = db.prepare('SELECT * FROM secret_credentials WHERE id = ? AND kind = ?')
      .get(profile.credentialId, 'ai-api-key');
    if (row) {
...
function getAiSettings() {
  ...
  const fallback = (profiles.find(profile => profile.isDefault) || profiles[0] || {}).id || '';
  const row = db.prepare('SELECT * FROM ai_settings WHERE id = 1').get() || {};
  const pick = column => valid.has(row[column]) ? row[column] : fallback;
  return {
    activeProfileId: pick('active_profile_id'),
    translationProfileId: pick('translation_profile_id'),
    rewriteProfileId: pick('rewrite_profile_id'),
    agentProfileId: pick('agent_profile_id'),
    updatedAt: Number(row.updated_at) || 0,
  };
}
...
function saveAiSettings(settings = {}) {
  ...
  const valid = new Set(listAiProfiles().map(profile => profile.id));
  const valueFor = (property) => {
    const requested = String(settings[property] || '').trim();
    if (requested && valid.has(requested)) return requested;
    return current[property] && valid.has(current[property]) ? current[property] : null;
  };
  const t = now();
  db.prepare(`
    INSERT INTO ai_settings (
      id, active_profile_id, translation_profile_id, rewrite_profile_id, agent_profile_id, updated_at
    ) VALUES (1, ?, ?, ?, ?, ?)
    ON CONFLICT(id) DO UPDATE SET
      active_profile_id = excluded.active_profile_id,
      translation_profile_id = excluded.translation_profile_id,
      rewrite_profile_id = excluded.rewrite_profile_id,
      agent_profile_id = excluded.agent_profile_id,
      updated_at = excluded.updated_at
  `).run(
    valueFor('activeProfileId'),
    valueFor('translationProfileId'),
    valueFor('rewriteProfileId'),
    valueFor('agentProfileId'),
    t,
  );
  return getAiSettings();
}

function getAiProfileConfigForPurpose(purpose = 'active') {
  const property = `${AI_PURPOSE_COLUMNS[purpose] ? purpose : 'active'}ProfileId`;
  const settings = getAiSettings();
  const id = settings[property] || settings.activeProfileId;
  return id ? getAiProfileConfig(id) : null;
}

function deleteAiProfile(id) {
  const profile = getAiProfile(id);
  if (!profile) return false;
  if (listAiProfiles().length <= 1) {
    const error = new Error('至少保留一个 AI 配置');
    error.statusCode = 409;
    throw error;
  }
  db.exec('BEGIN');
  try {
    db.prepare('DELETE FROM ai_profiles WHERE id = ?').run(profile.id);
    if (profile.credentialId) db.prepare('DELETE FROM secret_credentials WHERE id = ?').run(profile.credentialId);
    if (profile.isDefault) {
      const next = db.prepare('SELECT id FROM ai_profiles ORDER BY created_at ASC LIMIT 1').get();
      if (next) db.prepare('UPDATE ai_profiles SET is_default = 1 WHERE id = ?').run(next.id);
    }
    db.exec('COMMIT');
  } catch (error) {
    db.exec('ROLLBACK');
...
function assertUsername(username) {
  ...
  if (!/^[a-z0-9][a-z0-9._-]{2,63}$/.test(value)) {
    const err = new Error('用户名需为 3-64 位字母、数字、点、下划线或连字符');
    err.statusCode = 400;
    throw err;
  }
  return value;
...
/**
 * Fixed single-owner bootstrap. The configured password is only re-hashed when
 * it changes, so ordinary restarts keep existing sessions valid.
 */
function ensureOwnerUser({ username, password, displayName = 'Owner' }) {
  const cleanUsername = assertUsername(username);
  const cleanPassword = assertPassword(password);
  if (cleanPassword.length < 12) {
    const err = new Error('OWNER_PASSWORD 至少需要 12 个字符');
    err.statusCode = 400;
    throw err;
  }
  ...
  const existing = db.prepare(`
    ...
    ORDER BY CASE WHEN username = ? THEN 0 ELSE 1 END, created_at ASC
    LIMIT 1
  `).get(cleanUsername, cleanUsername);
  const t = now();
  if (!existing) {
    const record = createPasswordRecord(cleanPassword);
    const email = `${cleanUsername}@qmreader.local`;
    db.prepare(`
      INSERT INTO users (
        id, username, email, display_name, role, password_hash, password_salt,
    ...
    `).run(
      ...
      cleanUsername,
      email,
      publicAuthor(displayName || cleanUsername),
      record.hash,
      record.salt,
      t,
      t,
    ...
    const passwordChanged = !verifyPassword(cleanPassword, existing.password_salt, existing.password_hash);
    const record = passwordChanged ? createPasswordRecord(cleanPassword) : null;
    db.prepare(`
      UPDATE users
      SET username = ?,
          display_name = ?,
    ...
    `).run(
      cleanUsername,
      publicAuthor(displayName || cleanUsername),
      record ? record.hash : existing.password_hash,
      record ? record.salt : existing.password_salt,
      t,
      existing.id,
    );
  ...
  const owner = db.prepare('SELECT * FROM users WHERE username = ?').get(cleanUsername);
  db.prepare("UPDATE users SET role = 'user', updated_at = ? WHERE id <> ? AND role = 'admin'").run(t, owner.id);
...
function authenticateOwner(username, password) {
  ...
  const t = now();
  db.prepare('UPDATE users SET last_login_at = ?, updated_at = ? WHERE id = ?').run(t, t, row.id);
...
function closeDatabase() {
  try {
    ...
    // Tests may call cleanup more than once; production keeps the DB open.
  }
}
...
function setUserEntryState(userId, entryId, { read, starred, viewed } = {}) {
  ...
  if (!db.prepare('SELECT 1 FROM entries WHERE id = ?').get(id)) {
...
function markEntriesRead(userId, entryIds) {
  ...
  try {
    for (const id of ids) {
      if (exists.get(id)) stmt.run(userId, id, t, t);
    }
    db.exec('COMMIT');
  } catch (error) {
    db.exec('ROLLBACK');
...
function mergeUserEntryStates(userId, { read = [], starred = [], history = [] } = {}) {
  ...
  const add = (entryId, patch) => {
    const id = normalizeEntryId(entryId);
    if (!id || merged.size >= 10000) return;
    merged.set(id, { ...(merged.get(id) || {}), ...patch });
  };
  const t = now();
  for (const id of Array.isArray(read) ? read.slice(0, 5000) : []) add(id, { readAt: t });
  for (const id of Array.isArray(starred) ? starred.slice(0, 5000) : []) add(id, { starredAt: t });
  for (const item of Array.isArray(history) ? history.slice(0, 5000) : []) {
    if (!item || !item.entryId) continue;
    const viewedAt = Number(item.viewedAt);
    add(item.entryId, { viewedAt: Number.isFinite(viewedAt) && viewedAt > 0 ? viewedAt : t });
  }
  ...
  const upsert = db.prepare(`
    ...
    ON CONFLICT(user_id, entry_id) DO UPDATE SET
      read_at = COALESCE(user_entry_states.read_at, excluded.read_at),
      starred_at = COALESCE(user_entry_states.starred_at, excluded.starred_at),
      viewed_at = CASE
        WHEN excluded.viewed_at IS NULL THEN user_entry_states.viewed_at
        WHEN user_entry_states.viewed_at IS NULL THEN excluded.viewed_at
        ELSE MAX(user_entry_states.viewed_at, excluded.viewed_at)
      END,
      updated_at = excluded.updated_at
  `);
  db.exec('BEGIN');
  try {
    for (const [entryId, state] of merged) {
      if (!entryExists.get(entryId)) continue;
      upsert.run(
        ownerId,
        entryId,
        state.readAt || null,
  ...
  return getUserEntryStates(ownerId);
}
...
function getEntryStats(entryIds, viewer = null, { sparse = false } = {}) {
  const ids = [...new Set((entryIds || []).map(normalizeEntryId).filter(Boolean))];
  const out = {};
  // dense：兼容详情/写回；sparse：列表路径只返回有信号的条目，避免 N×空对象
  if (!sparse) {
  ...
  const ensure = (id) => {
    if (!out[id]) out[id] = emptyEntryStats(id);
    return out[id];
  };
  ...
  `).all(...ids)) {
    const stats = sparse ? ensure(row.entry_id) : out[row.entry_id];
    if (!stats) continue;
  ...
  `).all(...ids)) {
    const stats = sparse ? ensure(row.entry_id) : out[row.entry_id];
    if (stats) stats.favoriteCount = Number(row.count) || 0;
  }
  ...
  if (viewer && viewer.id) {
    ...
    `).all(...ids, viewer.id)) {
      const stats = sparse ? ensure(row.entry_id) : out[row.entry_id];
      if (stats) stats.reactionByMe = row.reaction || '';
    }
  }
  return out;
}
...
function normalizeEntry(row) {
  ...
  return {
    ...
    // 列表 hydrate 不选 content 时列为缺失 → 默认 ''
    content: row.content || '',
...
function upsertEntries(entries) {
  ...
  try {
    const t = now();
    const markOriginalStmt = db.prepare(`
      UPDATE entries
      SET original_fetched_at = COALESCE(?, original_fetched_at),
          original_fetch_attempted_at = COALESCE(?, original_fetch_attempted_at),
    ...
    // 软删粘性 + existing 字段：IN 预取（分块 400），去掉按条 SELECT
    ...
    for (let offset = 0; offset < ids.length; offset += 400) {
      ...
      for (const row of db.prepare(`
        ...
        WHERE id IN (${placeholders}) AND COALESCE(deleted_at, 0) != 0
      `).all(...chunk)) {
        softDeletedIds.add(row.id);
      }
      for (const row of db.prepare(`
        SELECT id, content, summary, image FROM entries
    ...
    for (const entry of list) {
      if (softDeletedIds.has(entry.id)) continue;
      const existing = existingById.get(entry.id);
      const incomingContent = entry.content || '';
      const existingContent = existing && existing.content ? existing.content : '';
      const content = !entry.forceContent && existingContent && plainTextLength(existingContent) > plainTextLength(incomingContent) + 240
        ? existingContent
        : incomingContent;
      ...
      if (entry.originalFetchedAt || entry.originalFetchAttemptedAt) {
        markOriginalStmt.run(
          entry.originalFetchedAt || null,
          entry.originalFetchAttemptedAt || null,
          entry.originalFetchedAt ? null : (entry.originalFetchError || null),
          t,
          entry.id,
        );
      }
...
/** List entries for a source (hydrate 内存 cache；不含 content，详情按需 getEntry). */
function listEntriesBySource(sourceId, limit = 200) {
  const sid = String(sourceId || '').trim();
  if (!sid) return [];
  // 与 getEntries / hydrateCacheFromDb 的 5000 对齐（源 limit 可达 2000+）
  const n = Math.max(1, Math.min(5000, Number.parseInt(limit, 10) || 200));
  return db.prepare(`
    SELECT e.id, e.source_id, e.title, e.link, e.author, e.published, e.published_ts,
           e.summary, e.image, e.audio_json, e.content_hash,
           e.original_fetched_at, e.original_fetch_attempted_at, e.original_fetch_error,
           e.deleted_at, e.deleted_by, e.deleted_reason,
           e.created_at, e.updated_at,
           t.title_zh, t.summary_zh, t.title_hash AS translation_title_hash
    FROM entries e
    LEFT JOIN entry_translations t ON t.entry_id = e.id
    WHERE e.source_id = ? AND COALESCE(e.deleted_at, 0) = 0
    ORDER BY e.published_ts DESC
    LIMIT ?
  `).all(sid, n).map(normalizeEntry);
}

function countEntriesBySource(sourceId) {
  const sid = String(sourceId || '').trim();
  if (!sid) return 0;
  const row = db.prepare(`
    SELECT COUNT(*) AS n FROM entries
    WHERE source_id = ? AND COALESCE(e.deleted_at, 0) = 0
  `.replace('e.deleted_at', 'deleted_at')).get(sid);
...
/** 软删元信息（reason 用于 left-watchlater 等自动剪枝再入列恢复） */
function getEntryDeleteMeta(entryId) {
  const id = normalizeEntryId(entryId);
  if (!id) return null;
  const row = db.prepare('SELECT deleted_at, deleted_by, deleted_reason FROM entries WHERE id = ?').get(id);
  if (!row || !Number(row.deleted_at)) return null;
  return {
...
/** 解除软删（个人精选再次收录等主动恢复路径） */
function clearEntrySoftDelete(entryId) {
  const id = normalizeEntryId(entryId);
  if (!id) return false;
  const t = now();
...
/** 按源软删全部未删条目（可恢复语义；不碰关联表与磁盘图） */
function softDeleteEntriesBySource(sourceId, { userId = '', reason = '' } = {}) {
  const sid = String(sourceId || '').trim();
  if (!sid) {
  ...
  return { sourceId: sid, deletedCount: Number(info.changes) || 0, deletedAt: t, mode: 'soft' };
}

/**
 * 按源硬删全部条目；依赖 PRAGMA foreign_keys + CASCADE 清翻译/点评/划线/对话等。
 * notifications / submission_requests 的 entry_id 为 ON DELETE SET NULL。
 */
function hardDeleteEntriesBySource(sourceId) {
  const sid = String(sourceId || '').trim();
  if (!sid) {
    const err = new Error('sourceId is required');
    err.statusCode = 400;
  ...
  const before = db.prepare(`
    SELECT COUNT(*) AS n FROM entries WHERE source_id = ?
  `).get(sid);
  const totalBefore = Number(before && before.n) || 0;
  let lastErr = null;
  // 与 likes-sync 并发写时可能 SQLITE_BUSY，短重试
  for (let attempt = 0; attempt < 5; attempt += 1) {
    try {
      const info = db.prepare(`DELETE FROM entries WHERE source_id = ?`).run(sid);
      return {
        sourceId: sid,
      ...
      if (!/busy|locked/i.test(msg) || attempt === 4) break;
      // 同步短等（store 无 async）
      const until = Date.now() + 80 * (attempt + 1);
      while (Date.now() < until) { /* spin */ }
    }
  }
  throw lastErr || new Error('hardDeleteEntriesBySource failed');
}

/** 同源同 link 去重：保留正文最长（同分保留 published 更具体）的一条，软删其余 */
function dedupeSourceEntriesByLink(sourceId, { reason = '同源同链重复' } = {}) {
  const sid = String(sourceId || '').trim();
  if (!sid) return { groups: 0, softDeleted: 0, kept: 0 };

  const rows = db.prepare(`
    SELECT id, link, published, published_ts, content, summary
    FROM entries
    WHERE source_id = ? AND COALESCE(deleted_at, 0) = 0
  `).all(sid);
  ...
  function contentLen(row) {
    return plainTextLength(row && (row.content || row.summary) || '');
  }
  ...
  let softDeleted = 0;
  let kept = 0;
  for (const [linkKey, group] of groups.entries()) {
    if (group.length <= 1) {
      kept += group.length;
      continue;
    }
    // 稳定 id：md5(sourceId|绝对link) 优先（与当前 feed normalizeItem 一致）
    const stableId = crypto.createHash('md5').update(`${sid}|${linkKey}`).digest('hex');
    const zenImportId = crypto.createHash('md5').update(`zen-import|${linkKey}`).digest('hex');

    group.sort((a, b) => {
      const lenDelta = contentLen(b) - contentLen(a);
      if (lenDelta) return lenDelta;
      // 同分：优先稳定绝对 link id，其次 zen-import（常有全文），再发布时间质量
      const rank = (row) => {
        if (row.id === stableId) return 300;
        if (row.id === zenImportId) return 200;
        if (String(row.id || '').length >= 32) return 50;
        return 0;
      };
    ...
    kept += 1;
    // 若组内有更好发布时间，写回 winner
    let bestIso = winner.published || '';
    let bestScore = publishedScore(bestIso);
    for (const row of group) {
      const sc = publishedScore(row.published);
      if (sc > bestScore) {
        bestScore = sc;
        bestIso = row.published;
      }
    }
    const bestTs = bestIso ? Date.parse(bestIso) : 0;
    if (bestTs > 0 && (winner.published !== bestIso || Number(winner.published_ts) !== bestTs)) {
      db.prepare(`
        UPDATE entries
        SET published = ?, published_ts = ?, updated_at = ?
        WHERE id = ?
      `).run(new Date(bestTs).toISOString(), bestTs, now(), winner.id);
    }
    // 合并正文：若 loser 更长，写回 winner（防止误删全文）
    for (const row of group.slice(1)) {
      if (contentLen(row) > contentLen(winner) + 40) {
        db.prepare(`
          UPDATE entries
          SET content = ?, summary = COALESCE(NULLIF(?, ''), summary), updated_at = ?
          WHERE id = ?
        `).run(row.content || '', row.summary || '', now(), winner.id);
        winner.content = row.content;
        winner.summary = row.summary || winner.summary;
      }
      const result = softDeleteEntry(row.id, { reason });
      if (result && !result.alreadyDeleted) softDeleted += 1;
    }
  }
  return { groups: groups.size, softDeleted, kept };
}
...
function saveSubmittedEntry(entry, { userId = null, author = '读者', note = '' } = {}) {
  ...
  // 个人精选再次粘贴同一链接：解除软删并强制覆盖正文/配图（公网「删后禁止复活」对自收录不适用）
  if (isEntryDeleted(entry.id)) {
    clearEntrySoftDelete(entry.id);
  }
  const toSave = { ...entry, forceContent: true };
  upsertEntries([toSave]);
  if (isEntryDeleted(entry.id)) {
    // 防御：理论上 clear + upsert 后不应仍软删
...
/** 从译文 content_json 头里抠首段中文，作列表概要兜底 */
function listSummarySnippetFromContentJson(contentJson) {
  try {
    const arr = typeof contentJson === 'string' ? JSON.parse(contentJson) : contentJson;
    if (!Array.isArray(arr)) return '';
    for (const pair of arr.slice(0, 12)) {
      if (!pair || pair.kind === 'media') continue;
      const t = String(pair.target || '')
        .replace(/<[^>]+>/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
      if (t.length >= 8 && /[\u3400-\u9fff]/.test(t)) return t.slice(0, 160);
    }
  } catch { /* ignore */ }
  return '';
}

/** { [entryId]: { titleZh, summaryZh } } — 列表默认中文标题+概要（有译文必尽量给中文） */
function getListTranslations(ids) {
  ...
  for (let offset = 0; offset < cleanIds.length; offset += 400) {
    ...
    const placeholders = chunk.map(() => '?').join(',');
    const rows = db.prepare(`
      SELECT t.entry_id, t.title_zh, t.summary_zh, t.title_hash, t.content_json, e.title
      FROM entry_translations t
      JOIN entries e ON e.id = t.entry_id
      WHERE t.entry_id IN (${placeholders})
        AND (
          (t.title_zh IS NOT NULL AND t.title_zh <> '')
          OR (t.summary_zh IS NOT NULL AND t.summary_zh <> '')
          OR (t.content_json IS NOT NULL AND t.content_json <> '' AND t.content_json <> '[]')
        )
    `).all(...chunk);
    for (const row of rows) {
      const titleZhRaw = String(row.title_zh || '').trim();
      // hash 对齐优先；有汉字的 title_zh 也下发（列表默认中文，避免略改英文标题后整卡回英文）
      const titleHashOk = titleZhRaw && row.title_hash === hashText(row.title || '');
      const titleHasHan = titleZhRaw && /[\u3400-\u9fff]/.test(titleZhRaw);
      const titleZh = (titleHashOk || titleHasHan) ? titleZhRaw : '';
      let summaryZh = String(row.summary_zh || '').trim();
      if (!summaryZh || !/[\u3400-\u9fff]/.test(summaryZh)) {
        const fromBody = listSummarySnippetFromContentJson(row.content_json);
        if (fromBody) summaryZh = fromBody;
      }
      if (!titleZh && !summaryZh) continue;
      out[row.entry_id] = {
        titleZh: titleZh || '',
        summaryZh: summaryZh || '',
      };
...
/** 兼容旧调用：仅 titleZh 字符串 map */
function getTitleTranslations(ids) {
  const meta = getListTranslations(ids);
  const out = {};
  for (const [id, row] of Object.entries(meta)) {
    if (row && row.titleZh) out[id] = row.titleZh;
  }
  return out;
}

/**
 * 列表专用：只查计数，稀疏返回（无资产的 id 不出现）。
 * 避免为 3000+ 条每人造一份 28 字段空对象 + 拉 preview/body。
 */
function getEntryAssetListSummaries(ids) {
  const cleanIds = [...new Set((ids || []).map(normalizeEntryId).filter(Boolean))];
  const out = {};
  ...
  function markLatest(asset, type, timestamp) {
    const t = Number(timestamp) || 0;
    if (t > asset.latestAt) {
      asset.latestAt = t;
      asset.latestTypes = [type];
    } else if (t && t === asset.latestAt && !asset.latestTypes.includes(type)) {
  ...
  for (let offset = 0; offset < cleanIds.length; offset += 400) {
    ...
    for (const row of db.prepare(`
      SELECT entry_id, asset_type, COUNT(*) AS count, MAX(updated_at) AS latest_at
      FROM entry_ai_asset_contributions
      WHERE entry_id IN (${placeholders})
        AND (
          (asset_type = 'translation' AND content_json IS NOT NULL AND content_json <> '' AND content_json <> '[]')
    ...
    `).all(...chunk)) {
      const asset = ensure(row.entry_id);
      const count = Number(row.count) || 0;
      if (row.asset_type === 'translation') {
        asset.translation = count > 0;
        asset.translationCount = count;
        markLatest(asset, 'translation', row.latest_at);
      } else if (row.asset_type === 'rewrite') {
        asset.rewrite = count > 0;
        asset.rewriteCount = count;
        markLatest(asset, 'rewrite', row.latest_at);
      }
    }
    ...
    `).all(...chunk)) {
      const asset = ensure(row.entry_id);
      if (!asset.translationCount) {
        asset.translation = true;
        asset.translationCount = 1;
        markLatest(asset, 'translation', row.updated_at);
      }
    }
    ...
    `).all(...chunk)) {
      const asset = ensure(row.entry_id);
      if (!asset.rewriteCount) {
        asset.rewrite = true;
        asset.rewriteCount = 1;
        markLatest(asset, 'rewrite', row.updated_at);
      }
    }
    ...
    `).all(...chunk)) {
      const asset = ensure(row.entry_id);
      asset.comments = Number(row.count) || 0;
      markLatest(asset, 'comments', row.latest_at);
    }
    ...
    `).all(...chunk)) {
      const asset = ensure(row.entry_id);
      asset.annotations = Number(row.count) || 0;
      markLatest(asset, 'annotations', row.latest_at);
    }
    ...
    `).all(...chunk)) {
      const asset = ensure(row.entry_id);
      asset.chatMessages = Number(row.count) || 0;
      markLatest(asset, 'chat', row.latest_at);
    }

    for (const row of db.prepare(`
      SELECT entry_id, COUNT(*) AS helpful_count
      FROM entry_asset_reactions
      WHERE entry_id IN (${placeholders}) AND reaction = 'helpful'
      GROUP BY entry_id
    `).all(...chunk)) {
      const asset = ensure(row.entry_id);
      asset.helpfulCount = Number(row.helpful_count) || 0;
    }
  }
...
function emptyEntryAssetSummary() {
  return {
    translation: false,
    rewrite: false,
    comments: 0,
...
function getEntryAssetSummaries(ids, { mode = 'full' } = {}) {
  if (mode === 'list') return getEntryAssetListSummaries(ids);

  const cleanIds = [...new Set((ids || []).map(normalizeEntryId).filter(Boolean))];
  const out = {};
  for (const id of cleanIds) {
    out[id] = emptyEntryAssetSummary();
  }
  if (!cleanIds.length) return out;
...
/** 思考笔记：一篇一条，Personal 模式本人私有（读后想法，Markdown 原文存储） */
function getEntryNote(entryId) {
  const row = db.prepare('SELECT * FROM entry_notes WHERE entry_id = ?').get(entryId);
...
/** 空正文视为删除笔记 */
function saveEntryNote(entryId, body) {
  const text = String(body || '');
  if (!text.trim()) {
    db.prepare('DELETE FROM entry_notes WHERE entry_id = ?').run(entryId);
    return null;
  }
  const t = now();
  db.prepare(`
    INSERT INTO entry_notes (entry_id, body, created_at, updated_at)
    VALUES (?, ?, ?, ?)
    ON CONFLICT(entry_id) DO UPDATE SET
      body = excluded.body,
      updated_at = excluded.updated_at
  `).run(entryId, text, t, t);
  return getEntryNote(entryId);
}
...
module.exports = {
  ...
  isEntryDeleted,
  getEntryDeleteMeta,
  clearEntrySoftDelete,
  closeDatabase,
  logRefreshJob,
  markNotificationsRead,
  markEntryOriginalFetchAttempt,
  markEntriesRead,
  mergeUserEntryStates,
  recordEntryView,
  ...
  setUserEntryState,
  softDeleteEntry,
  softDeleteEntriesBySource,
  hardDeleteEntriesBySource,
  dedupeSourceEntriesByLink,

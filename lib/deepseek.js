...
function getServerTranslationConfig(overrides = {}) {
  loadEnv();
  const stored = store.getAiProfileConfigForPurpose('translation');
  if (stored && String(stored.apiKey || '').trim()) {
    return getConfig({
      ...stored,
      temperature: 0.1,
      maxTokens: Math.max(Number(stored.maxTokens) || 0, 7000),
      ...overrides,
    });
  }
...
function getConfig(options = {}) {
  loadEnv();
  const implicitSavedProfile = !String(options.apiKey || '').trim()
    && !String(options.baseUrl || '').trim()
    && !String(options.model || '').trim()
    && !String(options.providerName || '').trim();
  if (implicitSavedProfile) {
    const stored = store.getAiProfileConfigForPurpose('active');
    if (stored && String(stored.apiKey || '').trim()) options = { ...options, ...stored };
  }

...
function shouldAutoFetchOriginal(entry) {
  if (typeof fetcher.shouldAutoFetchOriginal === 'function') {
    return fetcher.shouldAutoFetchOriginal(entry);
  }
...
async function prepareEntryForAiAsset(entry, reason = 'AI asset', { productHuntOfficialSite = true } = {}) {
  // Unified prepare path for HTTP AI assets + background auto-rewrite (was duplicated in server.js).
...
async function autoRewriteEntries(entries = [], { skipped = 'no changed entries' } = {}) {
  ...
  const savedRewriteProfile = store.getAiProfileConfigForPurpose('rewrite');
  const config = deepseek.getConfig(savedRewriteProfile && savedRewriteProfile.apiKey
    ? savedRewriteProfile
    : {
      provider: 'deepseek',

...
const { originalFetchPublicError } = require('./original-fetch-error');
...
loadCustomSources(store.listCustomSources());
...
function feedSiteUrl(feed, fallbackUrl) {
  const values = [feed && feed.link, fallbackUrl];
...
async function parseDiscoveredFeed(url) {
  const safeUrl = await assertPublicHttpUrl(url, { deadline: Date.now() + TIMEOUT_MS });
  const feed = await parseRssUrl(safeUrl);
  if (!feed || !Array.isArray(feed.items) || !feed.items.length) {
    const error = new Error('订阅地址没有返回可读取的文章');
  ...
  return {
    feed,
    feedUrl: safeUrl,
    siteUrl: feedSiteUrl(feed, safeUrl),
  };
}

async function discoverFeedUrl(input) {
  const directUrl = await assertPublicHttpUrl(String(input || '').trim(), { deadline: Date.now() + TIMEOUT_MS });
  try {
    const direct = await parseDiscoveredFeed(directUrl);
    return {
      feedUrl: direct.feedUrl,
      siteUrl: direct.siteUrl,
      name: String(direct.feed.title || new URL(direct.siteUrl).hostname).trim().slice(0, 80),
      description: stripHtml(direct.feed.description || '').slice(0, 240),
    };
  } catch (directError) {
    let html = '';
    try {
      html = await fetchText(directUrl, TIMEOUT_MS, MAX_HTML_RESPONSE_BYTES, { headers: BROWSER_HEADERS });
    } catch {
      throw directError;
    }
    ...
    const add = value => {
      try {
        const resolved = new URL(String(value || '').trim(), directUrl).toString();
        if (!candidates.includes(resolved)) candidates.push(resolved);
      } catch { /* skip malformed discovery links */ }
    };
    $('link[rel~="alternate"]').each((_, element) => {
      const type = String($(element).attr('type') || '').toLowerCase();
      if (/rss|atom|xml/.test(type)) add($(element).attr('href'));
    });
    $('a[href]').each((_, element) => {
      const href = String($(element).attr('href') || '');
      const label = String($(element).text() || '');
      if (/rss|atom|feed/i.test(`${href} ${label}`)) add(href);
    });
    for (const candidate of candidates.slice(0, 8)) {
      try {
        const discovered = await parseDiscoveredFeed(candidate);
        const pageTitle = $('title').first().text().replace(/\s+/g, ' ').trim();
        return {
          feedUrl: discovered.feedUrl,
          siteUrl: feedSiteUrl(discovered.feed, directUrl),
          name: String(discovered.feed.title || pageTitle || new URL(directUrl).hostname).trim().slice(0, 80),
          description: stripHtml(discovered.feed.description || '').slice(0, 240),
        };
      } catch { /* try the next advertised feed */ }
    }
    const error = new Error('没有从这个地址找到可读取的 RSS 或 Atom 订阅');
    error.statusCode = 422;
    throw error;
  }
}
...
function isEnabled(source) {
  if (source && source.custom) return source.enabled !== false;
  const o = runtime.state[source.id];
  return o && typeof o.enabled === 'boolean' ? o.enabled : source.enabled;
}

function setEnabled(id, enabled) {
  const source = SOURCES.find(item => item && item.id === id);
  if (source && source.custom) {
    const saved = store.setCustomSourceEnabled(source.id, enabled);
    source.enabled = saved ? saved.enabled : Boolean(enabled);
...
async function createCustomSource(input = {}) {
  const discovered = await discoverFeedUrl(input.url || input.feedUrl);
  const saved = store.saveCustomSource({
    name: String(input.name || discovered.name || '').trim(),
    feedUrl: discovered.feedUrl,
  ...
  return upsertCustomSource(saved);
}
...
async function fetchEntryOriginal(entry) {
  ...
  try {
    ...
    const publicError = originalFetchPublicError(error);
    const marked = store.markEntryOriginalFetchAttempt(entry.id, publicError.message);
...
function deleteSource(sourceId, {
  ...
  if (src.custom) {
    store.deleteCustomSource(id);
    removeCustomSource(id);
    delete runtime.state[id];
  }
